...
class StackOverflowHandler extends content_handler_1.ContentHandler {
  constructor() {
    super();
    this.name = 'stackoverflow';
  }
  parseText(element, title) {
    ...
    if (text) {
      ...
      if (votes) {
        newText.innerHTML = `<h2>${title}: ${votes} vote${votes === '1' ? '' : 's'}</h2>${text.innerHTML}`;
      }
    }
    return newText;
  }
    ...
    if (commentsDiv) {
      ...
      if (comments.length > 0) {
        ...
        comments.forEach(comment => {
          ...
          const date =
            comment.querySelector(`.relativetime-clean`)?.textContent;
  ...
  parseAuthors(element) {
    ...
    authors.forEach(author => {
      ...
      if (name && link && reputation && date) {
        ...
        newAuthor.innerHTML = `<a href="${link}"><b>${name}</b></a> - ${reputation} reputation - ${badges || 'no badge'} - ${date}`;
        if (isOwner) {
  ...
  shouldPreParse(_url, _dom) {
    return new URL(_url).hostname.endsWith('stackoverflow.com');
  }
