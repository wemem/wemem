...
Readability.prototype = {
  ...
  REGEXPS: {
    ...
    get okMaybeItsACandidate() {
      return new RegExp(
        `and|(?<!${this.articleNegativeLookAheadCandidates.source})article(?!-(${this.articleNegativeLookBehindCandidates.source}))|body|column|content|^(?!main-navigation|main-header)main|shadow|post-header|hfeed site|blog-posts hfeed|container-banners|menu-opacity|header-with-anchor-widget|commentOnSelection|highlight--with-header`,
        'i'
      );
    },

    positive:
      /article|body|content|entry|hentry|h-entry|main|page|pagination|post|text|blog|story|tweet(-\w+)?|instagram|image|container-banners|player|commentOnSelection/i,
    negative:
      /\bad\b|hidden|^hid$| hid$| hid |^hid |banner|combx|comment|com-|contact|footer|gdpr|masthead|media|meta|outbrain|promo|related|scroll|share|shoutbox|sidebar|skyscraper|sponsor|shopping|tags|tool|widget|controls|video-controls/i,
    extraneous:
    ...
    normalize: /\s{2,}/g,
    videos:
      /\/\/(www\.)?((dailymotion|youtube|youtube-nocookie|player\.vimeo|v\.qq|cdnapisec\.kaltura)\.com|(archive|upload\.wikimedia)\.org|player\.twitch\.tv|piped\.mha\.fi)/i,
  ...
  _cleanClasses: function (node) {
    ...
    if (
      node.className &&
      node.className.hasOwnProperty &&
      node.className.hasOwnProperty('_omnivore')
    ) {
      return;
    }
        ...
        if (href.indexOf('javascript:') === 0) {
          ...
          if (
            ...
            while (link.firstChild) {
              container.append(link.firstChild);
            }
    ...
    if (this.createImageProxyUrl !== undefined) {
      ...
      Array.from(images).forEach(image => {
        ...
        if (src && !dataUriRegex.test(src)) {
          ...
          const width = attToNumber(
            image.getAttribute('width') || image.style.width
          );
          const height = attToNumber(
          ...
          image.dataset.omnivoreOriginalSrc = absoluteSrc;
        }
      ...
      Array.from(elements).forEach(element => {
        ...
        for (let item of items) {
          ...
          if (w) {
            ...
            const proxySrc = this.createImageProxyUrl(
              this.toAbsoluteURI(link),
              +value
            );
  ...
  _simplifyNestedElements: function (articleContent) {
    ...
    while (node) {
      if (
        this.PLACEHOLDER_CLASSES.includes(node.className) ||
        this.hasEmbed(node) ||
        this.isEmbed(node)
      ) {
      ...
      if (
        ...
        this.PLACEHOLDER_CLASSES.includes(node.firstElementChild.className)
      ) {
        node = this._getNextNode(node);
        continue;
      }
  ...
  _prepArticle: async function (articleContent) {
    if (this._keepTables) {
      ...
      this._replaceNodeTags(
        this._getAllNodesWithTag(articleContent, ['table']).filter(
          t => !this._classesToPreserve.includes(t.className)
        ),
        'div'
      );
    }
    ...
    this._forEachNode(articleContent.children, function (topCandidate) {
      this._cleanMatchedNodes(topCandidate, function (node, matchString) {
        if (
          ...
          return !(
            node.tagName === 'IMG' && parseInt(node.getAttribute('width')) > 100
          );
        }
      });
    });
    ...
    this._removeNodes(
      ...
      function (paragraph) {
        ...
        const hasRedundantImage =
          imgCount === 1 && /^\+\d+$/g.test(this._getInnerText(paragraph));

        return (
          (totalCount === 0 &&
            !this._getInnerText(paragraph, false) &&
            !this.PLACEHOLDER_CLASSES.includes(paragraph.className)) ||
          hasRedundantImage
        );
      }
    );
    ...
    this._removeNodes(
      this._getAllNodesWithTag(articleContent, ['a']),
      function (anchor) {
        const possibleRedundantText =
          /view full|skip to content|Open in browser/gi;
        ...
        return (
          (possibleRedundantText.test(innerText) && innerText.length <= 30) ||
          className.includes(possibleRedundantClassName)
        );
      }
    );
  },
  ...
  _textSimilarity: function (textA, textB) {
    ...
    const { uniqTokensB, similarTokensB } = tokensB.reduce(
      (res, token) => {
    ...
    const lengthDistance =
      similarTokensB.join(' ').length / tokensA.join(' ').length;
    return (
      DISTANCE_WEIGHT * (1 - distanceB) + lengthDistance * (1 - DISTANCE_WEIGHT)
    );
  },
  ...
  _checkPublishedDate: function (node, matchString) {
    ...
    if (
      node.className === 'omnivore-published-date' &&
    ...
    let dateFound;
    const dateRegExpFound = this.REGEXPS.DATES_REGEXPS.find(regexp =>
      regexp.test(content)
    );
    if (dateRegExpFound) {
      dateFound = dateRegExpFound.exec(content)[0];
    } else if (this.REGEXPS.LONG_DATE_REGEXP.test(content)) {
      dateFound = this.REGEXPS.LONG_DATE_REGEXP.exec(content)[0].replace(
        /st|nd|rd|th/i,
        ''
      );
    } else if (this.REGEXPS.CHINESE_DATE_REGEXP.test(content)) {
      dateFound = this.REGEXPS.CHINESE_DATE_REGEXP.exec(content)[0]
        .replace(/年|月/g, '-')
        .replace(/日/g, '');
    }
    ...
    if (
      ...
      (this._someNodeAttribute(node, ({ value, name }) => {
        ...
        dateFound ||
        (/date/i.test(matchString) && !isNaN(publishedDateParsed))) &&
      this._isValidPublishedDate(node.textContent)
    ) {
      try {
  ...
  _grabArticle: async function (page) {
    ...
    while (true) {
      ...
      while (node) {
        ...
        if (
          this._checkByline(node, matchString) ||
          this._checkPublishedDate(node, matchString)
        ) {
          node = this._removeAndGetNext(node);
          continue;
        }
        ...
        if (shouldRemoveTitleHeader && this._headerDuplicatesTitle(node)) {
          ...
          if (
            (titleText !== headingText && titleText.includes(headingText)) ||
            this._someNodeAttribute(node, ({ value }) => value === 'headline')
          ) {
        ...
        if (stripUnlikelyCandidates) {
          if (
            (this.REGEXPS.unlikelyCandidates.test(matchString) ||
              ...
              this.REGEXPS.unlikelyCandidates.test(
        ...
        if (
          this.EMBEDS_CLASSES.includes(node.className) ||
          this.isEmbed(node) ||
          this.hasEmbed(node)
        ) {
          node = this._getNextNode(node);
      ...
      this._forEachNode(elementsToScore, function (elementToScore) {
        if (
          ...
          elementToScore.parentNode.tagName === undefined
        )
          return;
        ...
        this._forEachNode(ancestors, function (ancestor, level) {
          if (
            !ancestor.tagName ||
            !ancestor.parentNode ||
            ancestor.parentNode.tagName === undefined
          )
            return;

          if (ancestor.readability === undefined) {
      ...
      if (topCandidate === null || topCandidate.tagName === 'BODY') {
        ...
        while (parentOfTopCandidate.tagName !== 'BODY') {
          ...
          if (
            Array.from(parentOfTopCandidate.parentNode.childNodes).some(
              n => n.tagName === 'NAV'
            )
          ) {
            break;
      ...
      var siblingScoreThreshold = Math.max(
        10,
        (topCandidate.readability?.contentScore || 0) * 0.2
      );
      ...
      const headerNodes =
        ...
        this._getAllNodesWithTag(this._doc.documentElement, ['HEADER']);
      const alreadyExistingFigures = this._getAllNodesWithTag(articleContent, [
        'FIGURE',
      ]);
      headerNodes &&
        this._forEachNode(headerNodes, headerNode => {
          if (
            !headerNode ||
            !headerNode.readability ||
            !headerNode.readability.contentScore
          )
            return;
          ...
          this._forEachNode(figures, figure => {
            if (
              !this._someNode(
                alreadyExistingFigures,
                existingFigure => existingFigure === figure
              )
            ) {
              this.log('Prepending figure to the article', {
      ...
      if (this._debug)
        this.log('Article content pre-prep: ', {
      ...
      if (this._debug)
        this.log('Article content post-prep: ', {
          content: articleContent.innerHTML,
        });
      ...
      if (textLength < this._charThreshold) {
        ...
        if (this._flagIsActive(this.FLAG_STRIP_UNLIKELYS)) {
          ...
          if (
            ...
            !this._attempts[0].articleContent
          ) {
            return null;
          }
  ...
  _findJSONLDRecordById: function (parsedJSONLD, id, resultField) {
    if (
      !parsedJSONLD ||
      !parsedJSONLD['@graph'] ||
      !Array.isArray(parsedJSONLD['@graph'])
    )
      return;
  ...
  _getJSONLD: function (doc) {
    ...
    if (jsonLdElements.length) {
      try {
        ...
        const contents = jsonLdElements.map(el =>
        ...
        contents.forEach(content => {
          ...
          else parsedData['@graph'].push(parsedContent);
        });

        if (parsedData['@graph'].length === 1)
          parsedData = parsedData['@graph'][0];

        let parsedArticleInfo = parsedData;

        this.log(`JSONLD script found from ${scripts.length} scripts:`, {
          contents,
          parsed: parsedData,
        });
        ...
        if (
          ...
          this.log("Context hasn't matched!", {
        ...
        if (
          typeof parsedArticleInfo.name === 'string' &&
          typeof parsedArticleInfo.headline === 'string' &&
          parsedArticleInfo.name !== parsedArticleInfo.headline
        ) {
          ...
          var nameMatches =
            this._textSimilarity(parsedArticleInfo.name, title) > 0.75;
          var headlineMatches =
            this._textSimilarity(parsedArticleInfo.headline, title) > 0.75;
        ...
        if (parsedArticleInfo.author) {
          if (typeof parsedArticleInfo.author === 'string') {
            ...
            Array.isArray(parsedArticleInfo.author) &&
            parsedArticleInfo.author[0] &&
            typeof parsedArticleInfo.author[0].name === 'string'
          ) {
            ...
            metadata.byline = this._findJSONLDRecordById(
              parsedData,
              parsedArticleInfo.author['@id'],
              'name'
            );
          }
        }
        ...
        if (
          parsedArticleInfo.datePublished &&
          typeof parsedArticleInfo.datePublished === 'string'
        ) {
        ...
        if (typeof parsedArticleInfo.image === 'string') {
          ...
          Array.isArray(parsedArticleInfo.image) &&
          parsedArticleInfo.image[0] &&
          typeof parsedArticleInfo.image[0] === 'string'
        ) {
          ...
          if (typeof parsedArticleInfo.image.url === 'string')
            ...
            metadata.previewImage = this._findJSONLDRecordById(
              parsedData,
              parsedArticleInfo.image['@id'],
              'url'
            );
        }
        return metadata;
  ...
  _getArticleMetadata: function (jsonld) {
    ...
    var namePattern =
      /^\s*(?:(dc|dcterm|og|twitter|weibo:(article|webpage))\s*[\.:]\s*)?(author|creator|description|title|site_name|date|image)\s*$/i;
    ...
    if (metadata.previewImage) {
      ...
      try {
        metadata.previewImage = new URL(
          metadata.previewImage,
          new URL(this._baseURI).origin
        ).href;
      } catch {
    ...
    try {
      metadata.publishedDate =
        metadata.publishedDate &&
        new Date(this._unescapeHtmlEntities(metadata.publishedDate));
      if (
        !(metadata.publishedDate instanceof Date) ||
        isNaN(metadata.publishedDate)
      ) {
  ...
  _getInnerText: function (e, normalizeSpaces) {
    normalizeSpaces = normalizeSpaces === undefined ? true : normalizeSpaces;
  ...
  _createInstagramPostPlaceholder: function (element, postId) {
    ...
    post.dataset.instagramId = postId;
  ...
  _createPlaceholders: async function (e) {
    for (const element of Array.from(e.getElementsByTagName('a'))) {
      ...
      if (
        ...
        (element.parentNode && element.parentNode.className === 'tweet')
      ) {
        ...
        if (
          ...
          while (
            tweetParent &&
            tweetParent.children.length === 1 &&
            tweetParent.parentNode
          ) {
          ...
          if (
            tweetParent &&
            tweetParent.className.includes('twitter-tweet') &&
            tweetParent.parentNode
          ) {
            tweetParent.parentNode.replaceChild(tweet, tweetParent);
          }
          ...
          element.parentNode &&
          element.parentNode.className === 'tweet'
        ) {
      ...
      if (element.href.includes('instagram.com/p')) {
        ...
        if (
          Array.isArray(match) &&
          typeof match[2] === 'string' &&
          parentClassIncludes(element, 'instagram')
        ) {
    ...
    Array.from(e.getElementsByTagName('iframe')).forEach(element => {
      ...
      if (element.dataset.tweetId) {
        ...
        tweet.innerText = 'Tweet placeholder';
        tweet.className = 'tweet-placeholder';
        tweet.dataset.tweetId = element.dataset.tweetId;
  ...
  hasEmbed: function (element) {
    ...
    const classes = this.EMBEDS_CLASSES.reduce(
      (res, cur, i) => `${i > 0 && res + ','}.${cur}`,
      ''
    );
  ...
  _getLinkDensity: function (element) {
    ...
    this._forEachNode(element.getElementsByTagName('a'), function (linkNode) {
      ...
      linkNode.parentNode.tagName !== 'FIGCAPTION' &&
  ...
  _clean: function (e, tag) {
    ...
    this._removeNodes(this._getAllNodesWithTag(e, [tag]), function (element) {
      ...
      if (
        ...
        element.children[0].tagName === 'BLOCKQUOTE'
      ) {
  ...
  _fixLazyImages: function (root) {
    this._forEachNode(
      this._getAllNodesWithTag(root, ['img', 'picture', 'figure', 'svg']),
      function (elem) {
        ...
        if (imgHeight && imgWidhth && imgHeight === imgWidhth) {
          if (elem.tagName.toLowerCase() === 'svg') {
            if (imgHeight <= 21) {
              this.log(
                `Removing small square SVG: ${imgWidhth}x${imgHeight}`,
            ...
            this.log(
              `Removing small square image: ${imgWidhth}x${imgHeight}`,
              `className: ${elem.className}`,
              `src: ${elem.src}`
            );
            elem.remove();
        ...
        if (elem.dataset.lazySrc) {
          elem.setAttribute('src', elem.dataset.lazySrc);
        }
        ...
        if (
          ...
          if (
            elem.className &&
            this.REGEXPS.lazyLoadingElements.test(elem.className)
          ) {
            elem.remove();
          }
          return;
  ...
  _cleanConditionally: function (e, tag) {
    ...
    this._removeNodes(this._getAllNodesWithTag(e, [tag]), function (node) {
      ...
      if (
        ...
        node.children[0].tagName.toLowerCase() === 'picture'
      ) {
        return false;
      }
      ...
      if (weight + contentScore < 0) {
        this.log('Cleaning Conditionally by weight', {
          text: node.innerText,
          className: node.className,
          children: Array.from(node.children).map(ch => ch.tagName),
        });
        return true;
      }
      ...
      if (this._getCharCount(node, ',') < 10) {
        ...
        const textHasEmoji =
          Array.from(innerText.matchAll(emojiRegex)).length > 0;
        ...
        var haveToRemove =
          ...
          ((img > 1 &&
            ...
            (!isList &&
              weight < 25 &&
              linkDensity > 0.2 &&
              !this.CLASSES_TO_SKIP.some(c => parentClasses.contains(c))) ||
            ...
            (weight >= 25 &&
              linkDensity > 0.5 &&
              !(node.className === 'tweet' && linkDensity === 1)) ||
  ...
  _isOmnivoreNode: function (node) {
    ...
    while (walk) {
      if (
        walk.className &&
        walk.className.startsWith &&
        walk.className.startsWith(prefix)
      ) {
        return true;
      }
      if (
        walk.className &&
        walk.className.hasOwnProperty &&
        walk.className.hasOwnProperty(prefix)
      ) {
        return true;
      }
  ...
  _cleanHeaders: function (e) {
    ...
    let nodeToRemove = this._findNode(headingNodes, node => {
      ...
      return (
        ...
        (WEIGHT_MEANING_HEADERS.includes(node.tagName.toLowerCase()) &&
          this._getClassWeight(node) < 0)
      );
    });
    if (nodeToRemove) {
    ...
    return (
      ...
      (!node.hasAttribute('aria-hidden') ||
        !(
          ...
          node.parentElement.tagName.toLowerCase() !== 'figcaption'
        ) ||
        (node.className &&
  ...
  _isProbablyNavigation: function (node) {
    ...
    for (const child of children) {
      if (
        navRelatedClasses.test(child.className) &&
        node.getElementsByTagName('a') !== null
      ) {
        return true;
      }
    }
  ...
  _getLanguage: async function (content, locale, languageCode) {
    try {
      ...
      const languages = (await detect(content, { isHTML: true })).languages;
  ...
  parse: async function () {
    ...
    const [author, publishedDateFromAuthor] =
      extractPublishedDateFromAuthor(byline);
    const publishedDate =
      metadata.publishedDate ||
    ...
    const language = await this._getLanguage(
      content,
      metadata.locale,
      this._languageCode
    );
