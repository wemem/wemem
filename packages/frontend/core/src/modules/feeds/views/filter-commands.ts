...
export function filterSortAndGroupCommands(
  ...
  query: string
): [CommandCategory, FeedSearchCommand[]][] {
  ...
  if (query) {
    ...
    return Object.entries(groups) as [CommandCategory, FeedSearchCommand[]][];
  }
}
...
export const getCommandScore = (command: FeedSearchCommand, search: string) => {
  if (search.trim() === '') {
    return 1;
  }
